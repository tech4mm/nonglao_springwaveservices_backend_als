                                 ...
                                 @foreach ($this->filteredUsers as $user)
                                    <div wire:click="$set('receiverId', {{ $user->id }})"
                                            x-on:click="$nextTick(() => {
                                                ...
                                                class="chat-item cursor-pointer px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-black flex items-center gap-3 {{ $receiverId === $user->id ? 'bg-gray-200 dark:bg-gray-700 font-semibold' : '' }}">
                                        @if ($user->user_picture)
                                            <img src="{{ asset('storage/' . $user->user_picture) }}" 
                                                alt="User" 
